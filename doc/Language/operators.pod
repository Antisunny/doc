...
Hyper operators apply a given operator enclosed by C<«> and C<»> or C<\>\>> and C<\<\<> to one or two lists, returning the resulting list. The pointy part of
C<«> or C<»> has to point to the shorter list. A list with just one element is
fine too. If one of the lists is shorter then the other, the operator will cycle
over the shorter list until all elements of the longer list are processed.

    say (1,2,3) »*» 2;       # (2,4,6)
    say (1,2,3,4) »~» <a b>; # (1a 2b 3a 4b)
    say (1,2,3) »+« (4,5,6); # (5 7 9)
    say (1,2,3) «~» <R A Y>; # (1R 2A 3Y)
...
the blunt end at the list to be operated on.
    ...
    my @wisdom = '', '0', 0, Nil;
    say !« @wisdom; # [True False True True]
...
%outer »+» %inner;   only keys of %outer

%outer «+« %inner;   only keys of %inner
=end table

    my %outer = 1,2,3 Z=> <a b c>; # {"1" => "a", "2" => "b", "3" => "c"}
    my %inner = 1,2 Z=> <x z>;     # {"1" => "x", "2" => "z"}
    say %outer «~» %inner;         # {"1" => "ax", "2" => "bz"}
    say %outer «~« %inner;         # {"1" => "ax", "2" => "bz"}
    say %outer »~« %inner;         # {"1" => "ax", "2" => "bz", "3" => "c"}

    my %Joe = 'A' => q[Apple Juice], 'B' => q|Banana Mash|, 'D' => Q[Map];
    my %Paul = 'D' => q[Used Clothes], 'S' => Q/Dirty Shoes/;
    say %Joe «~« %Paul;
    # gets {"D" => "MapUsed Clothes", "S" => "Dirty Shoes"}
